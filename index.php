...
// Processar formulário de inscrição na newsletter
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['email']) && !isset($_POST['usuario'])) {
    ...
    if (!empty($email) && filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        $stmt = mysqli_prepare($conexao, $check_query);
        ...
        if (mysqli_stmt_num_rows($stmt) == 0) {
            ...
            if (mysqli_stmt_execute($stmt_insert)) {
                $_SESSION['newsletter_success'] = "Email cadastrado com sucesso! Você receberá nossas novidades.";
            } else {
                $_SESSION['newsletter_error'] = "Erro ao cadastrar email. Tente novamente.";
            }
            mysqli_stmt_close($stmt_insert);
        } else {
            $_SESSION['newsletter_info'] = "Este email já está cadastrado em nossa newsletter.";
        }
        mysqli_stmt_close($stmt);
    } else {
        $_SESSION['newsletter_error'] = "Por favor, insira um email válido.";
    }
}
...
// Verificar mensagens de reset de senha
...
// Verificar mensagens da newsletter
if (isset($_SESSION['newsletter_success'])) {
    echo "<div class='alert alert-success alert-dismissible fade show' role='alert'>
            <i class='fas fa-check-circle mr-2'></i>{$_SESSION['newsletter_success']}
            <button type='button' class='btn-close' data-bs-dismiss='alert'></button>
          </div>";
...
if (isset($_SESSION['newsletter_error'])) {
    echo "<div class='alert alert-danger alert-dismissible fade show' role='alert'>
            <i class='fas fa-exclamation-triangle mr-2'></i>{$_SESSION['newsletter_error']}
            <button type='button' class='btn-close' data-bs-dismiss='alert'></button>
          </div>";
    unset($_SESSION['newsletter_error']);
}

if (isset($_SESSION['newsletter_info'])) {
    echo "<div class='alert alert-info alert-dismissible fade show' role='alert'>
            <i class='fas fa-info-circle mr-2'></i>{$_SESSION['newsletter_info']}
            <button type='button' class='btn-close' data-bs-dismiss='alert'></button>
          </div>";
    unset($_SESSION['newsletter_info']);
}
              ...
              <div class="col-12 col-md-6">
                ...
                <form action="php/processa_login.php" method="POST">
                  ...
                  <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <a href="#!" class="text-body" data-bs-toggle="modal" data-bs-target="#resetSenhaModal">Esqueceu a senha?</a>
                  </div>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="resetSenhaModalLabel">Esqueci minha senha</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <p>Digite seu email cadastrado. O administrador será notificado para gerar uma nova senha.</p>
        <form action="php/processa_reset_senha.php" method="POST">
          <div class="mb-3">
            <label for="resetEmail" class="form-label">Email</label>
            <input type="email" class="form-control" id="resetEmail" name="email" required>
          </div>
          <button type="submit" class="btn btn-primary">Solicitar Reset</button>
        </form>
      </div>
    </div>
  </div>
        ...
        <div id="heroCarousel" class="carousel slide carousel-fade" data-bs-ride="carousel" data-bs-interval="5000" role="region" aria-label="Slider de Herói">
            ...
            <div class="carousel-inner" role="listbox">
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    $caption = $row['titulo'] ?? ''; // Usar titulo em vez de legenda
...
document.addEventListener('DOMContentLoaded', function() {
    // Fazer uma requisição AJAX para contar a visita
