...
if($('.datanew').length>0){$('.datanew').DataTable({"bFilter":true,"sDom":'fBtlpi','pagingType':'numbers',"ordering":true,"language":{search:' ',sLengthMenu:'_MENU_',searchPlaceholder:"Procurar...",info:"_START_ - _END_ of _TOTAL_ itens",},initComplete:(settings,json)=>{$('.dataTables_filter').appendTo('#tableSearch');$('.dataTables_filter').appendTo('.search-input');},});}
...
$(document).ready(function() {
    ...
    $('.open-siderbar').click(function() {
        $('.siderbar-view').toggleClass('open');
    });

    // Fecha o sidebar
    $('.btn-closed').click(function() {
        $('.siderbar-view').removeClass('open');
    });

    // Alterna o modo escuro
    $('#notification_switch3').change(function() {
